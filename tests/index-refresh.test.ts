...
import type { Cataloger, CatalogedTool } from "../src/upstream/index.js";
...
describe("change-detection", () => {
  describe("captureSnapshot", () => {
    ...
    test("generates consistent hashes for same schema", () => {
      const tool1 = createMockTool("tool", "server", { type: "object", foo: 1 });
      const tool2 = createMockTool("tool", "server", { type: "object", foo: 1 });
    ...
    test("generates different hashes for different schemas", () => {
      const tool1 = createMockTool("tool", "server", { type: "object", foo: 1 });
      const tool2 = createMockTool("tool", "server", { type: "object", foo: 2 });
  ...
  describe("detectChanges", () => {
    ...
    test("detects modified tools", () => {
      const beforeTools = [createMockTool("read_file", "fs", { type: "object", v: 1 })];
      const afterTools = [createMockTool("read_file", "fs", { type: "object", v: 2 })];
    ...
    test("detects multiple changes at once", () => {
      const beforeTools = [
        ...
        createMockTool("tool_b", "fs", { v: 1 }),
        createMockTool("tool_c", "fs"),
      ];
      const afterTools = [
        createMockTool("tool_a", "fs"),
        createMockTool("tool_b", "fs", { v: 2 }), // modified
...
describe("IndexRefreshManager", () => {
  ...
  test("emits refresh:error on failure", async () => {
    const errorCataloger = {
      ...mockCataloger,
      refreshAllTools: mock(() => Promise.reject(new Error("Connection failed"))),
    } as unknown as Cataloger;
