...
describe("MonitorClient", () => {
    ...
    const server = createServer((socket) => {
      socket.on("data", (data) => {
        ...
        if (command.startsWith("ping")) {
          ...
          const limit = parts[1] ? parseInt(parts[1], 10) : 100;
          response = JSON.stringify({
            status: "success",
  ...
  describe("error handling", () => {
    test("handles malformed response", async () => {
      ...
      await monitorClient.connect();
      await expect(monitorClient.ping()).rejects.toThrow("Failed to parse response");
    ...
    test("handles socket error", async () => {
      ...
      await monitorClient.connect();
      await expect(monitorClient.ping()).rejects.toThrow("Connection closed before receiving response");
