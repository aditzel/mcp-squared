...
import { DEFAULT_CONFIG, DetailLevelSchema, type McpSquaredConfig } from "@/config/schema";
...
describe("Security filtering in discovery", () => {
  ...
  test("requiresConfirmation flag can be added to tools", () => {
    ...
    const toolWithConfirmation: ToolSummary & { requiresConfirmation?: boolean } = {
      name: "write_file",
  ...
  test("tool without requiresConfirmation flag is valid", () => {
    const toolWithoutConfirmation: ToolSummary & { requiresConfirmation?: boolean } = {
      name: "read_file",
...
describe("list_namespaces functionality", () => {
  ...
  test("namespace info with tools has correct structure", () => {
    ...
    expect(namespace.tools).toEqual(["read_file", "write_file", "list_directory"]);
